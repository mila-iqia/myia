...
from myia.impl.impl_interp import fit, shape, sum, exp, log, setattr
from myia.front import myia
from myia.lib import record
import numpy
...
@grad_test((record(a=1, b=2, c=3),))
def test_setattr(r):
    # Note: we cannot use the syntax r.a *= 3, because the gradient is
    # tested against running the Python version of the code. However,
    # records are immutable, so the Python version will fail.
    r = setattr(r, 'a', r.a * 3)
    r = setattr(r, 'b', r.b / r.a)
    r = setattr(r, 'c', r.c + r.b)
    return r
...
@grad_test(((M51, Z11), M15, M11),
           ((N31, N11), N13, N11))
def test_square_cost(params, x, y):
    y_hat = logistic_regression(params, x)
    return sum((y_hat - y) ** 2)


def logistic_regression_record(params, x):
    act = x @ params.w + params.b
    return sigmoid(act)


@grad_test((record(w=M51, b=Z11), M15, M11),
           (record(w=N31, b=N11), N13, N11))
def test_square_cost_record(params, x, y):
    y_hat = logistic_regression_record(params, x)
    return sum((y_hat - y) ** 2)
