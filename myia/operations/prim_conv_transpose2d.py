...
from ..lib import (AbstractArray, force_pending, SHAPE, AbstractScalar,
                   standard_prim, TYPE, u64pair_typecheck, AbstractTuple)
from .. import xtype
...
async def infer_conv_transpose2d(
    ...
    output_padding = tuple(self.require_constant(e, argnum=f'"4:output_padding[{edx}]"')
                    for edx, e in enumerate(output_padding.elements))
    groups = self.require_constant(groups, argnum='5:groups')
    dilation = tuple(self.require_constant(e, argnum=f'"6:dilation[{edx}]"')
                     for edx, e in enumerate(dilation.elements))
    _, c_out_per_group, kh, kw = await force_pending(weight.xshape())
    c_out = c_out_per_group * groups
    h_out = int(
        (h_in - 1) * stride[0] - 2 * padding[0] + dilation[0] * (kh - 1) + output_padding[0] + 1
    )
    w_out = int(
        (w_in - 1) * stride[1] - 2 * padding[1] + dilation[1] * (kw - 1) + output_padding[1] + 1
    )
